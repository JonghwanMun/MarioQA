...
function layer:__init(uparams, utables, opt)
  ...
  self:_createInitState(1) -- will be lazily resized later during forward passes
	...
	print(string.format('Sentence Encoder backend    : %s ',self.backend))
	print(string.format('Sentence Encoder GRU size   : %d (sentence vector size)',self.rnn_size))
...
function layer:updateGradInput(input, gradOutput)
   ...
   self.gradInput:resizeAs(seq:typeAs(gradOutput)):zero()

   -- go backwards and lets compute gradients
   local dstate = {[self.tmax+1] = self.init_state} -- this works when init_state is all zeros
   local doutput = torch.Tensor(batch_size, self.rnn_size):typeAs(gradOutput):zero()
   for t=self.tmax,1,-1 do
      ...
      local dword = self.lookup_tables[t]:backward(self.lookup_tables_inputs[t], dcore[1])

      dstate[t] = dcore[2]
      self.gradInput[t] = dword
   end
