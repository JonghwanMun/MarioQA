...
cmd = torch.CmdLine()
cmd:text()
cmd:text('Train an Average Pooling Baseline Model')
cmd:text()
cmd:text('Options')
...
if string.len(opt.start_from) > 0 then  -- finetuning the model
   ...
   net.c3d_net:add( nn.Transpose({1,2}, {2,3}, {3,4}) )
   net.c3d_net:add( nn.View(-1, opt.clip_feat_dim) )
   net.c3d_net:add( nn.Mean(1) )
...
local function lossFun()
   ...
   for bi=1,opt.batch_size do
      ...
      --print(string.format('dQuestion : %f',dquestion:float():norm()))
