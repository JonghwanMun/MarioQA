...
cmd = torch.CmdLine()
cmd:text()
cmd:text('Train a VideoQA Temporal Attention Model')
cmd:text()
cmd:text('Options')
...
cmd:option('-ft_continue', 1,'whether maintain the epoch and iteration of checkpoint or not (-1 = don\'t)')
...
cmd:option('-num_step', 2, 'number of steps for temporal attention')
...
-- Optimization: for the C3D Model
...
local thin_att = net.att:clone()
for t=1,opt.num_step do 
   thin_att.core_t_att[t]:share(net.att.core_t_att[t], 'weight', 'bias')
   net_utils.sanitize_gradients(thin_att.core_t_att[t])
end
...
local function eval_split(split, evalopt)
   ...
   while true do
      ...
      if val_clips_use >= 0 and n >= val_clips_use then break end -- we've used enough clips
      print('-----------------------------------------------------------------------------------')
   end
...
local function lossFun()
   net.question_encoder:training(); net.c3d_net:training(); net.att:training(); net.classify:training()
   grad_qe_params:zero(); grad_c3d_params:zero(); grad_att_params:zero(); grad_cls_params:zero()
   ...
   for bi=1,opt.batch_size do
      ...
      local clip_feat = net.c3d_net:forward(data.clips[bi])  -- (clip_feat_dim, frames, height, width)
      local att_feat  = net.att:forward({clip_feat, qst_feat})
      local pred      = net.classify:forward({att_feat, qst_feat})
      local loss      = net.crit:forward(pred, data.answers[{{bi}}])
      batch_loss      = batch_loss + loss
      forward_time    = forward_time + timer:time().real - ft
      ...
      local dclip_feat, dqst_feat2 = unpack( net.att:backward({clip_feat, qst_feat}, datt_feat) )
      local dclip     = net.c3d_net:backward(data.clips[bi], dclip_feat)
      local dqst      = net.question_encoder:backward({data.questions[{ {},{bi} }], data.question_length[{ {bi} }]}, dqst_feat1+dqst_feat2)
      backward_time   = backward_time + timer:time().real - bt

      if loss >= 10 then print(string.format('Oops!! Loss is bigger than 10 (%.3f) (%s)',loss,data.infos[bi]['video_path'])) end
   end
...
while true do
   ...
   if ((iter+1) % 3 == 0 and epoch < 2) or iter == opt.max_epoch*every_epoch then
      ...
      local thin_att_weight= thin_att:getWeight()
      local att_weight= net.att:getWeight()
      if torch.sum( thin_att_weight - att_weight ) ~= 0 then
         print('!!!!! Model is not saved correctly, ATT')
      end
   end
