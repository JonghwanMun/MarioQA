...
cmd = torch.CmdLine()
cmd:text()
cmd:text('Train a Global Context Mmodel')
cmd:text()
cmd:text('Options')
...
if string.len(opt.start_from) > 0 then  -- finetuning the model
   ...
   -- attatch global context embedding layer
   net.c3d_net:add(nn.View(-1):setNumInputDims(4))
   net.c3d_net:add(nn.Linear(143360,opt.clip_feat_dim))
   net.c3d_net:add(nn.ReLU())
   net.c3d_net:add(nn.Dropout(opt.drop_prob))
   net.c3d_net:add(nn.Linear(opt.clip_feat_dim,opt.clip_feat_dim))
   net.c3d_net:add(nn.ReLU())
   net.c3d_net:add(nn.Dropout(opt.drop_prob))
...
local function eval_split(split, evalopt)
   ...
   while true do
      ...
      local pred = net.classify:forward({clip_feat,qst_feat})
...
local function lossFun()
   net.question_encoder:training(); net.c3d_net:evaluate(); net.classify:training()
   grad_qe_params:zero(); grad_c3d_params:zero(); grad_cls_params:zero()
   ...
   for bi=1,opt.batch_size do
      ...
      local dclip_feat, dqst_feat = unpack( net.classify:backward({clip_feat,qst_feat}, dpred) )
...
while true do
   ...
   if  ((iter+1) % 10 == 0 or epoch < 3) or iter == opt.max_epoch*every_epoch then
